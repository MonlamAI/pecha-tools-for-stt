...
function failToLogin(code: string) {
  // [Reason] Audit login failures with reason codes only — never tokens or OAuth payloads
  logAuthEvent(
    { event: "login_failure", reason: code, path: "/callback" },
    "User login failed"
  );
...
export async function GET(req: NextRequest) {
  // [Reason] Bind request logging context so auth events include requestId automatically
  const loggingIdentity = await resolveAuthLogIdentity(req, {
    path: "/callback",
    auditInvalidSession: false,
  });

  return runWithLoggingContext(loggingIdentity, req, async () => {
    ...
    // [Reason] Audit successful login with user id/email only — never session or Google tokens
    logAuthEvent(
      {
        event: "login_success",
        userId: user.id,
