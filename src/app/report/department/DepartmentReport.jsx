...
const DepartmentReport = ({ departments }) => {
  ...
  // ✅ FIX: normalize datetime-local to ISO (MATCHES MAIN VERSION)
  ...
  const moveMonth = (delta) => {
    ...
    startTransition(() => {
      setDates({
        from: toDatetimeLocal(newFrom),
        to: toDatetimeLocal(newTo),
      });
    });
  };
  ...
  useEffect(() => {
    console.log("Active group users:", usersStatistic[activeGroupId]?.length);
    console.log("All groups keys:", Object.keys(usersStatistic));
  }, [usersStatistic, activeGroupId]);
    ...
    async function fetchGroup() {
      ...
      try {
        const qs = new URLSearchParams({
          ...
          // ✅ FIX: normalized dates
          from: normalizeToISO(dates.from),
          to: normalizeToISO(dates.to),
        }).toString();
  ...
  const handleLoadDepartmentTotals = async () => {
    ...
    try {
      ...
      // The API returns objects keyed by group ID: { "123": [...], "124": [...] }
      setUsersStatistic((prev) => ({ ...prev, ...data.users }));
      setReviewersStatistic((prev) => ({ ...prev, ...data.reviewers }));
      setFinalReviewersStatistic((prev) => ({ ...prev, ...data.finalReviewers }));

      setDeptTotalsLoaded(true);
    } catch (e) {
      console.error("Error loading department totals", e);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex gap-2">
            {groups.map((g) => (
              <button
                key={g.id}
                type="button"
                className={`btn btn-sm ${String(activeGroupId) === String(g.id)
                    ? "btn-primary"
                    : "btn-ghost"
                  }`}
                onClick={() =>
                  startTransition(() => setActiveGroupId(String(g.id)))
                }
              >
                {g.name}
              </button>
